...
import {
  ...
  Diamond,
  GoogleCloud,
} from '../../base/icons/src/public/billing'
...
const KeyValue = ({
  ...
  return (
    <div className={cn('flex', textColor)}>
      ...
      <div className={cn('system-sm-regular ml-2 mr-0.5', textColor)}>
        {label}
      </div>
      {tooltip && (
        <Tooltip asChild popupContent={tooltip} popupClassName="w-[200px]">
          <div className="flex size-4 items-center justify-center">
...
const style = {
  [SelfHostedPlan.community]: {
    ...
    btnStyle:
      'bg-components-button-secondary-bg hover:bg-components-button-secondary-bg-hover border-[0.5px] border-components-button-secondary-border text-text-primary',
  ...
  [SelfHostedPlan.premium]: {
    ...
    btnStyle:
      'bg-third-party-aws hover:bg-third-party-aws-hover border border-components-button-primary-border text-text-primary-on-surface shadow-xs',
    values: 'text-text-secondary',
    tooltipIconColor: 'text-text-tertiary',
  },
  [SelfHostedPlan.enterprise]: {
    ...
    bg: 'border-effects-highlight bg-[#fc0373] text-text-primary-on-surface',
    btnStyle:
      'bg-white bg-opacity-96 hover:opacity-85 border-[0.5px] border-components-button-secondary-border text-[#fc0373] shadow-xs',
    values: 'text-text-primary-on-surface',
    tooltipIconColor: 'text-text-primary-on-surface',
  },
}
const SelfHostedPlanItem: FC<Props> = ({ plan }) => {
  ...
  return (
    <div
      className={cn(
        `relative flex w-[374px] flex-col overflow-hidden rounded-2xl
      border-[0.5px] hover:border-effects-highlight hover:shadow-lg hover:backdrop-blur-[5px]`,
        style[plan].bg,
      )}
    >
      <div>
        <div
          className={cn(
            isEnterprisePlan
              ? 'z-1 absolute bottom-0 left-0 right-0 top-0 bg-price-enterprise-background'
              : '',
          )}
        ></div>
        {isEnterprisePlan && (
          <div className="z-15 absolute -left-[90px] -top-[104px] size-[341px] rounded-full bg-[#09328c] opacity-15 mix-blend-plus-darker blur-[80px]"></div>
        )}
        {isEnterprisePlan && (
          <div className="z-15 absolute -bottom-[72px] -right-[40px] size-[341px] rounded-full bg-[#e2eafb] opacity-15 mix-blend-plus-darker blur-[80px]"></div>
        )}
      </div>
      <div className="relative z-10 min-h-[559px] w-full p-6">
        <div className=" flex min-h-[108px] flex-col gap-y-1">
          ...
          <div className="flex items-center">
            <div
              className={cn(
                'system-md-semibold uppercase leading-[125%]',
                style[plan].title,
              )}
            >
              {t(`${i18nPrefix}.name`)}
          ...
          <div className={cn(style[plan].description, 'system-sm-regular')}>
        ...
        <div className="my-3">
          <div className="flex items-end">
            <div
              className={cn(
                'shrink-0 text-[28px] font-bold leading-[125%]',
                style[plan].price,
              )}
            >
              {t(`${i18nPrefix}.price`)}
            ...
            {!isFreePlan && (
              <span
                className={cn(
                  'ml-2 py-1 text-[14px] font-normal leading-normal',
                  style[plan].priceTip,
                )}
              >
        ...
        <div
          className={cn(
            'system-md-semibold flex h-[44px] cursor-pointer items-center justify-center rounded-full px-5 py-3',
            style[plan].btnStyle,
          )}
        ...
        <div className={cn('system-sm-semibold mb-2 mt-6', style[plan].values)}>
        ...
        {isPremiumPlan && (
          <div className="mt-[68px]">
            ...
            <span
              className={cn(
                'system-xs-regular mt-2',
                style[plan].tooltipIconColor,
              )}
            >
